...
class Trainer:
    ...
    def train_epoch(self, epoch, train_loader, loss_fn, optimizer, device):
        ...
        for i, (features, f1_cut, f2_cut, offset, random_pick, l1, l2) in enumerate(train_loader):

            features = features.to(device)

            ground_truth = np.zeros([features.shape[0], len(self.classes), features.shape[2], features.shape[3]])
            ...
            for j, (lab1, lab2) in enumerate(zip(l1, l2)):
                ...
                #two overlapping vocalization types
                    ...
                    ground_truth[j][self.classes.index(lab1)] = f1_cut[j]
                    ground_truth[j][self.classes.index(lab2)] = f2_cut[j]
            ...
            if i > (len(train_loader)-2):
                self.writer.add_image(
                    tag="train" + "/net_input_overlap_"+l1[0]+"__"+l2[0],
                    img_tensor=prepare_img(
                        features[0], num_images=self.n_summaries, file_name=l1[0]+"__"+l2[0],
                    ),
                    global_step=epoch,
                )
                self.writer.add_image(
                    tag="train" + "/gt1_" + str(l1[0]),
                    img_tensor=prepare_img(
                        f1_cut[0].unsqueeze(dim=0), num_images=self.n_summaries, file_name=l1[0],
                    ),
                    global_step=epoch,
                )
                self.writer.add_image(
                    tag="train" + "/gt2_" + str(l2[0]),
                    img_tensor=prepare_img(
                        f2_cut[0].unsqueeze(dim=0), num_images=self.n_summaries, file_name=l2[0],
                    ),
                    global_step=epoch,
                )
                self.writer.add_image(
                    tag="train" + "/net_output_separated_"+l1[0]+"__"+l2[0],
                    img_tensor=prepare_img(
                        output[0].squeeze(dim=0), num_images=len(self.classes), file_name=l1[0]+"__"+l2[0], classes=self.classes,
                    ),
                    global_step=epoch,
                )
    ...
    def test_epoch(self, epoch, test_loader, loss_fn, device, phase="val"):
        ...
        with torch.no_grad():
            ...
            for i, (features, f1_cut, f2_cut, offset, random_pick, l1, l2) in enumerate(test_loader):
                ...
                if phase != "test":
                    num_imgs = len(test_loader)-2
                else:
                    num_imgs = len(test_loader)-11

                if i > num_imgs:
                    self.writer.add_image(
                        tag=phase + "/net_input_overlap_"+l1[0]+"__"+l2[0],
                        img_tensor=prepare_img(
                            features[0], num_images=self.n_summaries, file_name=l1[0]+"__"+l2[0],
                        ),
                        global_step=epoch,
                    ...
                    self.writer.add_image(
                        tag=phase + "/net_output_separated_"+l1[0]+"__"+l2[0],
                        img_tensor=prepare_img(
                            output[0].squeeze(dim=0), num_images=len(self.classes), file_name=l1[0]+"__"+l2[0], classes=self.classes,
                        ),
                        global_step=epoch,
