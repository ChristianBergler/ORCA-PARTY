...
class OverlappedPreGeneratedDataset(AudioDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._logger.info("Number of overlapping files in {}: {}".format(self.dataset_name, len(self.file_names)))
    ...
    def __getitem__(self, idx):
        ...
        final, f1_cut, f2_cut, offset, random_pick, l1, l2 = pk
        final = torch.from_numpy(final).unsqueeze(0)

        return final.float(), f1_cut, f2_cut, offset, random_pick, l1, l2


"""
Extracts the year and tape from the given audio filename (filename structure: call-label_ID_YEAR_TAPE_STARTTIME_ENDTIME)
"""
def get_overlapped_types(file):
    try:
        file_wo_ending = file[:len(file)-2]
        types = file_wo_ending.split("__")
        call1 = types[0].split("_")[0]
        call2 = types[1].split("_")[0]

        return call1 + "_" + call2 
...
class OverlappedPreGeneratedDatasetCsvSplit(CsvSplit):
    """
    Get a dictonary of all overlapping combinatorial pairs. The number of elements is (N-1) smaller than the actual correct
    number of combinations, because the dictonary combines all combinations with the call itself, e.g. N2_N2, N1_N1, etc.
    except the garbage class
    """
    def split_fn(self, files: Iterable[Any]):
        ...
        for file in files:
            try:
                key = get_overlapped_types(file)
                if key is not None:
                    overlap_categories[key] += 1
                else:
                    n_files -= 1
            except IndexError:
                n_files -= 1
        ...
        overlap_categories_lst = list(overlap_categories)

        """
        Helper class which creates a mapping (per fraction) in order to handle added overlapped categories and number of files per overlapped category
        """
        class Mapping:
            def __init__(self):
                self.count = 0
        ...
        for overlapping_cat in overlap_categories_lst:
            missing_files = {
            ...
            for _split, _n_files in missing_files.items():
                r -= _n_files
                if r < 0:
                    mappings[_split].add(overlapping_cat, overlap_categories[overlapping_cat])
                    break
            assert r < 0, "Should not get here"

        splits = defaultdict(list)
        for file in files:
            tape = get_overlapped_types(file)
            if tape is not None:
